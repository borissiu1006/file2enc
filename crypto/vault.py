...
#   0x04 → Seq2Enc (GRU neural keystream) — experimental
...
VAULT_MAGIC   = b"F2EV"          # 4-byte magic
VAULT_VERSION = 0x01             # bump when format changes
SALT_SIZE     = 16
...
def _parse_vault(raw: bytes) -> tuple[bytes, bytes, bytes, bytes]:
    ...
    if version != VAULT_VERSION:
        raise ValueError(
            ...
            f"but this build supports version {VAULT_VERSION}.\n"
